...
function addIframe(cur){
	...
	if (!isHas) {
		var tab = "<a href='javascript:;' style='cursor:pointer' class='content-tab active' data-id='"+h+"'>"+ label +" <i class='icon-font'>&#xe617;</i></a>";
	...
	for(var i=0; i<menu.length; i++){   
		...
		try{
			if(item.isHeader == "1"){
				...
				if(item.childMenus == ""){
					if (item.isHeader == "2"){
						str = "<li><a id='song' href='"+item.url+"'><i class='icon-font'>"+item.icon+"</i><span>"+item.name+"</span></a></li>";
						$(parent).append(str);
					}else if (item.isHeader == "5"){
						str = "<li><a id='user' href='"+item.url+"'><i class='icon-font'>"+item.icon+"</i><span>"+item.name+"</span></a></li>";
						$(parent).append(str);
					}else if (item.isHeader == "7"){
						str = "<li><a id='account' href='"+item.url+"'><i class='icon-font'>"+item.icon+"</i><span>"+item.name+"</span></a></li>";
						$(parent).append(str);
					}else {
...
$(function(){
	...
	var menu = [{"id":"1","name":"主菜单","parentId":"0","url":"","icon":"","order":"1","isHeader":"1","childMenus":[
					{"id":"3","name":"曲库添加","parentId":"1","url":"http://"+url+"/music/admin/addHouse","icon":"&#xe604;","order":"1","isHeader":"0","childMenus":""},
						{"id":"4","name":"歌单管理","parentId":"3","url":"http://"+url+"/music/userSong","icon":"","order":"1","isHeader":"2","childMenus":""},
						{"id":"5","name":"提升权限","parentId":"3","url":"http://"+url+"/music/jurisdiction","icon":"","order":"1","isHeader":"0","childMenus":""},
					{"id":"6","name":"用户管理","parentId":"1","url":"","icon":"&#xe602;","order":"1","isHeader":"0","childMenus":[
						{"id":"7","name":"个人中心","parentId":"6","url":"http://"+url+"/music/user","icon":"","order":"1","isHeader":"5","childMenus":""},
						{"id":"8","name":"注销账号","parentId":"6","url":"http://"+url+"/music/account","icon":"","order":"1","isHeader":"7","childMenus":""}
					]},
					{"id":"9","name":"root用户管理","parentId":"1","url":"","icon":"&#xe602;","order":"1","isHeader":"0","childMenus":[
						{"id":"10","name":"申请管理","parentId":"6","url":"http://"+url+"/music/root/applyAdmi","icon":"","order":"1","isHeader":"","childMenus":""},
						{"id":"11","name":"留言管理","parentId":"6","url":"http://"+url+"/music/root/messageAdmin","icon":"","order":"1","isHeader":"0","childMenus":""},
						{"id":"12","name":"用户管理","parentId":"6","url":"http://"+url+"/music/root/userAdmin","icon":"","order":"1","isHeader":"0","childMenus":""}
					]}
				]}
];
	initMenu(menu,$(".side-menu"));
	$(".side-menu > li").addClass("menu-item");
	$(".side-menu li:nth-child(2)").addClass("House");
	$(".side-menu li:nth-child(6)").addClass("root");
	$('#account').parent().removeClass("House");
	/*获取菜单icon随机色*/
	//getMathColor();
